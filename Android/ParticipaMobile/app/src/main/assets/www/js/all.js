...
var Utils = new Utils();
//var slider = new PageSlider($('maincontainer'));
...
function loadElements(){
	...
	// JA: PER PODER TREBALLAR
    deviceIdentifier = "e0ab022f-de02-4ffa-b2a5-bb4efbb9f6cf"
...
function settingsInit(){
	...
	if(DEBUG_MODE) console.log("Settings screen loading...!");
	...
	function reapplySettingsRessources(){
        ...
        // Council , Languages i Notifications parts
		$('#settings_council').html(user_settings.appsettings.settingsCouncilTitle);
		$('#settings_language').html(user_settings.appsettings.settingsLanguageTitle);
		$('#settings_categories').html(user_settings.appsettings.categoryHeaderTitle);
		$('#settings_council_txt').html(user_settings.council.name);
        $('#settings_language_txt').html(user_settings.language.name);
		...
		$('#prodbtn').attr("src", typeService == "PRO" ? $c.localgraphics.prdbtn_hover : $c.localgraphics.prdbtn);
		$('#prebtn').attr("src", typeService == "PRE" ? $c.localgraphics.prebtn_hover : $c.localgraphics.prebtn);
		$('#devbtn').attr("src", typeService == "DES" ? $c.localgraphics.devbtn_hover : $c.localgraphics.devbtn);


		//alert(typeService + " current: " + $c.settings.service_url_currenttype);
        if (typeService === 'PRO')
        {
			$('#settings_role').addClass("hide");
        }
        else if (typeService == 'PRE')
        {
        	$('#settings_role').removeClass("hide");
            $('#devbtn').addClass("hide");
        }
	...
	$('#tokenId_copy').bind('click',function(){
        copyToClipboard(deviceIdentifier);
    });

	Materialize.showStaggeredList('#list_main_content_area');
}
...
function welcomeInit(welcome_information){
	...
	$('.welcome-banner').src = welcome_information.BANNER;
	$('.welcome-banner').src = welcome_information.WELCOMETXT;
	$('.welcome-banner').src = welcome_information.BANNER;
	...
	// Load template
	var welcomeItemsValues = [];

    var welcomeItem = {};
	welcomeItem.Banner = welcome_information.BANNER;
	welcomeItem.Tittle = welcome_information.WELCOMETXT;
	welcomeItem.Summary  = welcome_information.SUBTITLE;
	welcomeItemsValues.push(welcomeItem);
    ...
    var dataSource = new kendo.data.DataSource({
        ...
        transport: {
           read: function (op) {
               op.success(welcomeItemsValues);
           }
        }
    });

    $("#content_area").kendoListView({
        dataSource: dataSource,
        selectable: "single",
        //dataBound : setItemClickEvent,
        template  : kendo.template($("#templateWelcome").html()),
        endlessScroll: false
    });

    var listView = $("#content_area").data("kendoListView");

    $('.welcome-banner').bind('click', function(){
        preloadCategory();						// To category screen.
    });


	//slider.slidePage($('.pContent'));
...
function productlistInit(theme_products){
	...
	try
	{
		...
		var ProductListValues = [];

		for(var i = 0; i < products.length; i++)
		{
			var ProductItem = {};
			ProductItem.Id = 'prolistitem' + i;
			ProductItem.Tittle = products[i].PRDTITLE;
			ProductItem.Description = products[i].PRDDESCRIPTION;
			ProductItem.Icon = products[i].PRDICON;
			ProductItem.data = products[i];
			ProductListValues.push(ProductItem);
		}
		...
		$("#list_content_area").kendoListView({
			...
			selectable: "single",
			dataBound : setItemClickEvent,
			template  : kendo.template($("#templateProducts").html()),
			endlessScroll: true,
	...
	{
		if(DEBUG_MODE) console.log("----> Error in productlistInit");
	}

	Materialize.showStaggeredList('#list_content_area');
}

function loadProducListBehaviour (productItem)
{
	var product = productItem.data;
...
function getCouncilBanner(id)
{
    var service_url = $c.settings.service_url_base + 'COUNCILS(' + id + ')?$format=json';
    if(DEBUG_MODE) console.log("Service call: " + service_url);
...
function councilsListInit(councils){
    ...
    for(var i = 0; i < councils.length; i++)
    {
        ...
        councilItem.Icon = getCouncilBanner(councilItem.Id);
        //councilItem.icon = councils[i].council.welcome_logo;  // JA: s'ha de fer servir la propietat logo en el futur
    ...
    $("#list_content_area").kendoListView({
        ...
        selectable: "single",
        dataBound : setItemClickEvent,
        template  : kendo.template($("#templateCouncils").html())
